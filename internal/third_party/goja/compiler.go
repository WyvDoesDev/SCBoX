...
import (
	"fmt"
	"scbox/internal/third_party/goja/token"
	"sort"

	"scbox/internal/third_party/goja/ast"
	"scbox/internal/third_party/goja/file"
	"scbox/internal/third_party/goja/unistring"
)
...
// allocatesStash reports whether this scope produces a runtime stash, mirroring
// exactly the conditions under which the VM creates one:
//   - enterFunc/enterFunc1 is emitted when stashSize>0 || argsInStash, and
//   - enterBlock/enterFuncBody when stashSize>0 || extensible(dynamic),
// where stashSize>0 means some binding is placed in the stash — which happens
// for an explicitly stashed binding (needStash) or, when the scope is dynamic
// (dynLookup forces allInStash), for any binding at all.
//
// This must be used to count nesting levels for stash access (see
// finaliseVarAlloc). The naive `needStash || isDynamic()` over-counts a scope
// that only has dynLookup set (propagated up the chain by a nested direct
// eval()) but holds no bindings and is not extensible — e.g. the synthetic body
// scope of a function declaring a default/destructured parameter. Such a scope
// allocates NO stash, so counting it shifts every loadStashLex level up by one,
// making it read a non-existent stash and panic with "index out of range". (This
// is the goja default-parameter + nested-eval miscompilation.)
func (s *scope) allocatesStash() bool {
	if s.dynamic || s.argsInStash || s.needStash {
		return true
	}
	if s.isDynamic() {
