...
import (
	...
	"scbox/internal/third_party/sourcemap"
)
		...
		if strings.HasPrefix(urlStr, "data:application/json") {
			...
			if sourceURL := file.ResolveSourcemapURL(self.file.Name(), urlStr); sourceURL != nil {
				if self.opts.sourceMapLoader != nil {
					...
					// SCBoX patch: upstream read the source map from the host
					// filesystem here (os.ReadFile(sourceURL.Path)) whenever the URL
					// used the file/empty scheme. In a sandbox compiling UNTRUSTED
					// JavaScript that is an arbitrary host-file-read primitive and a
					// file-existence oracle: a sample need only embed a comment like
					// `//# sourceMappingURL=file:///etc/passwd` to make the parser
					// touch a host path of its choosing. We never read the host for
					// source maps. Inline `data:` source maps are handled above and
					// stay in memory; an embedder that genuinely needs file-backed
					// maps must opt in explicitly via WithSourceMapLoader.
					return nil
				}
			}
		}
