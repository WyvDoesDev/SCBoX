...
import (
	...
	"scbox/internal/third_party/regexp2"
...
func compileRegexp2(src string, multiline, dotAll, ignoreCase, unicode bool) (*regexp2Wrapper, error) {
	...
	// SCBoX patch: bound backtracking time. regexp2 is a backtracking engine, and
	// goja routes ECMAScript regexps that use features Go's RE2 cannot (back-
	// references, lookbehind, etc.) to it. Upstream leaves MatchTimeout at
	// regexp2.DefaultMatchTimeout (effectively infinite), so an untrusted pattern
	// with catastrophic backtracking (ReDoS) hangs the process forever — a single
	// in-flight Go match cannot be broken by the embedder's VM interrupt/budget,
	// so this otherwise defeats every analysis deadline. A finite cap makes the
	// match abort and return "no match" (handled cleanly by the callers here).
	regexp2Pattern.MatchTimeout = Regexp2Timeout

	return &regexp2Wrapper{rx: regexp2Pattern}, nil
}

// Regexp2Timeout bounds a single regexp2 (backtracking-engine) match. It is a
// package var so an embedder can tune it; the default is comfortably under
// typical per-execution analysis deadlines while still aborting ReDoS quickly.
var Regexp2Timeout = 2 * time.Second
