...
func (cfg *Config) expandUser(field string, moreFields bool) (prefix, rest string) {
	...
	// Not the current user; try via "HOME <name>" from the supplied Env.
	//
	// SCBoX patch: upstream fell back to os/user.Lookup(name) here, which on
	// Linux reads the real /etc/passwd. That let an untrusted script enumerate
	// real host users and their home directories via `~root`, `~daemon`, etc. —
	// a host-info leak and sandbox-detection vector. We never consult the host:
	// if the embedder's Env does not define this user's home, the tilde prefix is
	// left unexpanded (the standard shell behavior for an unknown user).
