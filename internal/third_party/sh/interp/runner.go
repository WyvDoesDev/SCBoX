...
import (
	...
	"scbox/internal/third_party/sh/internal"
	"scbox/internal/third_party/sh/pattern"
...
func (r *Runner) fillExpandConfig(ctx context.Context) {
	...
	r.ecfg = &expand.Config{
		...
		ProcSubst: func(ps *syntax.ProcSubst) (string, error) {
			...
			// SCBoX patch: the upstream implementation creates a real named FIFO on
			// the host (mkfifo + os.OpenFile + os.Remove under r.tempDir, which
			// defaults to the real os.TempDir()). That is a sandbox escape: an
			// untrusted script using `<( … )` / `>( … )` would create, write, read,
			// and delete real files on the host disk. We never spawn real
			// subprocesses — every exec is faked — so no real FIFO is needed. Run
			// the inner statements synchronously in a subshell with their stdio
			// wired to in-memory endpoints (so the commands are still executed,
			// recorded, and analyzed) and hand the outer command a synthetic
			// /dev/fd path. The outer command's open of that path is itself routed
			// through the faked OpenHandler, so no host descriptor is ever created.
			r2 := r.subshell(true)
			switch ps.Op {
			case syntax.CmdIn:
				// <(cmd): the inner command writes; the (faked) outer command would
				// read. Discard the inner output but still run the commands.
				r2.stdout = io.Discard
			case syntax.CmdOut:
				// >(cmd): the (faked) outer command writes; the inner command reads.
				// The subshell already inherits the runner's (empty) stdin, so we
				// only need to let the inner command's output flow to stdout.
				r2.stdout = r.origStdout
			default:
...
func (r *Runner) open(ctx context.Context, path string, flags int, mode os.FileMode, print bool) (io.ReadWriteCloser, error) {
	// SCBoX patch: upstream bypassed the open handler and called os.OpenFile
	// directly for interpreter-created FIFO temp files (matched by tempDir +
	// "sh-interp-" prefix). That was a host-write primitive — a script doing
	// `echo x > $TMPDIR/sh-interp-y` reached the real filesystem. We no longer
	// create real FIFOs (see ProcSubst), so every open now goes through the
	// faked OpenHandler with no exception.
