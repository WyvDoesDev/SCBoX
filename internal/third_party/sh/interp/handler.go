...
// SCBoX patch: the package-level checkStat/findExecutable/findFile resolve PATH
// entries with a real os.Stat, so builtins that perform a lookup (`type`,
// `command -v`, `source`/`.`) become a host-filesystem existence oracle — e.g.
// `type node` stats /usr/bin/node on the real host, and `command -v /etc/passwd`
// reveals whether that host file exists. The functions below route every lookup
// through the embedder's stat handler (the in-memory FS) instead, so a lookup can
// only observe the virtual filesystem. They mirror checkStat's exec/dir checks.
func (r *Runner) statFind(ctx context.Context, checkExec bool) findAny {
	return func(dir, file string, _ []string) (string, error) {
		if !filepath.IsAbs(file) {
			file = filepath.Join(dir, file)
		}
		info, err := r.statHandler(ctx, file, true)
		if err != nil {
			return "", err
		}
		m := info.Mode()
...
// lookPath resolves an executable via the faked stat handler (never the host).
func (r *Runner) lookPath(ctx context.Context, file string) (string, error) {
	return lookPathDir(r.Dir, r.writeEnv, file, r.statFind(ctx, true))
}

// scriptFromPath resolves a script (exec or not) via the faked stat handler.
func (r *Runner) scriptFromPath(ctx context.Context, file string) (string, error) {
	return lookPathDir(r.Dir, r.writeEnv, file, r.statFind(ctx, false))
}
