...
func (r *Runner) builtin(ctx context.Context, pos syntax.Pos, name string, args []string) (exit exitStatus) {
	...
	case "pwd":
		...
		pwd := r.envGet("PWD")
		// SCBoX patch: `pwd -P` upstream called filepath.EvalSymlinks(pwd), which
		// runs os.Lstat/os.Readlink against the real host filesystem — a host
		// existence/symlink oracle reachable as `PWD=/some/host/path pwd -P`. The
		// sandbox's virtual FS has no symlinks, so the physical path equals the
		// logical PWD; evalSymlinks becomes a no-op and never touches the host.
		_ = evalSymlinks
		r.outf("%s\n", pwd)
	case "cd":
		var path string
	...
	case "source", ".":
		...
		path, err := r.scriptFromPath(ctx, args[0])
		if err != nil {
	...
	case "read":
		...
		var line []byte
		var err error
		// SCBoX patch: `read -s` (silent) upstream read the process's REAL stdin
		// via term.ReadPassword(syscall.Stdin), bypassing the embedder's faked
		// stdin and blocking on the host operator's terminal — an information /
		// availability escape. The only effect of -s is to suppress terminal echo,
		// which is meaningless in the sandbox (there is no real terminal), so a
		// silent read uses the faked stdin exactly like a normal read.
		_ = silent
